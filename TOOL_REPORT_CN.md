...
`straighten_path(path, cost_raster)` 是路径回溯和路径平滑之间新增的关键后处理步骤。
...
```
1. 从起点开始
2. 从终点向前逐个检查：能否从当前点直线到达该点？
   （使用 Bresenham 直线算法列举直线经过的所有栅格单元，
    检查是否全部为有效单元——无 NaN/Inf/负值）
3. 找到最远的可达点后，将其设为下一个路径点
4. 从该路径点重复步骤 2，直到到达终点
```

**Bresenham 直线算法**：用于在栅格上列举两点之间直线经过的所有单元。这是一种经典的整数算法，效率极高，被广泛应用于计算机图形学和路径规划中。
...
拉直后（任意角度直线）：
(0,0) → (9,5)
仅 2 个路径点——一条从起点到终点的直线
```

**重要说明**：
- 拉直不会改变已计算的栅格路径代价（`total_cost`），代价仍然反映原始栅格路径
- 拉直后的路径可能穿过与原始栅格路径略有不同的单元，但所有穿过的单元都保证是有效的（非障碍物）
- 在障碍物边缘，拉直会保留必要的转弯路径点，确保路径不穿越障碍
- 拉直是自动进行的，无需用户设置参数
