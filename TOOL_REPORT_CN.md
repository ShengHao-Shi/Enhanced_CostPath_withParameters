...
| `0.3` | 中等拉直 | 每次最多跳过 30%，消除大部分锯齿但保留更多中间点 |
| `0.5` | 较强拉直 | 每次最多跳过 50%，明显减少路径点 |
| `1.0`（默认） | 完全拉直 | 不限制跳过距离，尽可能以最少的路径点连接起终点 |
...
straighten_factor=0.3：约 5 个路径点（中等拉直）
straighten_factor=1.0：2 个路径点——一条从起点到终点的直线
```
...
| 6 | Straighten Factor | GPDouble (0.0–1.0) | 数值输入（可选，默认 1.0） | 路径拉直因子 |
...
- **含义**：控制路径拉直的激进程度。该参数决定拉直算法每次可以向前"看"多远来寻找可以直线连接的路径点。
  ...
  - `0.1–0.3`：**轻微拉直**。每次最多跳过路径总长度的 10%–30%，仅消除小范围的锯齿，保留大部分中间路径点。适用于 NODATA 区域密集、希望路径紧贴栅格的场景。
  - `0.3–0.7`：**中等拉直**。消除大部分锯齿弯折，路径可以较自由地沿任意角度前进，但仍保留一些中间路径点。
  - `1.0`：**完全拉直**。不限制前瞻距离，尽可能以最少的路径点连接起终点。产生最直的路径，但在复杂地形中可能显得过于激进。
...
| NODATA 区域密集，需要保守拉直 | `straighten_factor=0.1–0.3`（减少前瞻距离） |
| 希望路径尽量直但安全 | `straighten_factor=0.5–0.7`（中等拉直） |
| 完全拉直（默认行为） | `straighten_factor=1.0` |
