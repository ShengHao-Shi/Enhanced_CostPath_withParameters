...
3. [核心算法模块 enhanced_lcp.py](#3-核心算法模块-enhanced_lcppy)
   ...
   - 3.9 [任意角度路径拉直](#39-任意角度路径拉直)
   - 3.10 [路径平滑——Chaikin 角切割](#310-路径平滑chaikin-角切割)
   - 3.11 [栅格文件读写辅助函数](#311-栅格文件读写辅助函数)
...
| **任意角度路径拉直（Any-Angle Straightening）** | 八方向栅格连通导致路径只能沿 8 个方向移动，产生阶梯状弯折；拉直后路径可沿任意角度前进 |
| **路径平滑（Path Smoothing）** | 拉直后的路径在必要转弯处仍有锐角，需要后处理产生平滑曲线 |
...
`straighten_path(path, cost_raster, straighten_factor=1.0)` 是路径回溯和路径平滑之间的关键后处理步骤。

**问题背景**：在栅格网格上，路径只能沿 8 个方向移动（上、下、左、右及四个对角）。当理想路径方向不与这 8 个方向之一对齐时（例如 30°），路径必须在两个相邻方向之间交替移动，产生"闪电状"锯齿弯折。例如，从 (0,0) 到 (10,5) 的理想直线路径在网格上会被迫交替使用"南"和"东南"方向，形成阶梯形。

**解决方案**：路径拉直使用 **视线检查（Line-of-Sight）** 方法消除不必要的中间路径点。核心思想是"拉紧绳子"——如果两个路径点之间的直线不穿过任何障碍物，则中间的所有网格路径点都是多余的，可以跳过。

**算法流程**：

```
1. 从起点开始
2. 根据 straighten_factor 确定最大前瞻距离
   （max_skip = straighten_factor × (路径总点数 - 1)）
3. 在前瞻范围内，从最远点向前逐个检查：能否从当前点直线到达该点？
   （使用 Supercover 直线算法列举直线经过的所有栅格单元，
    检查是否全部为有效单元——无 NaN/Inf/负值）
4. 找到最远的可达点后，将其设为下一个路径点
5. 从该路径点重复步骤 3，直到到达终点
```

**Supercover 直线算法**：用于在栅格上列举两点之间直线经过的 **所有** 单元。与标准 Bresenham 算法不同，Supercover 算法在对角步进时会同时检查对角两侧的相邻单元（角落单元），确保不会遗漏任何 NODATA 或障碍物单元。这防止了拉直后的路径穿越 NODATA 像素。

```
标准 Bresenham (0,0)→(1,1)：仅检查 (0,0) 和 (1,1)
Supercover    (0,0)→(1,1)：检查 (0,0)、(0,1)、(1,0) 和 (1,1)
```

**`straighten_factor` 参数**：

| 取值 | 含义 | 效果 |
|------|------|------|
| `0.0` | 不拉直 | 输出路径与原始栅格路径完全相同（转为浮点坐标） |
| `0.05` | 极轻微拉直 | 每次最多跳过路径总长度的 5%，精细消除局部小锯齿 |
| `0.1` | 轻微拉直 | 每次最多跳过路径总长度的 10%，只消除小范围锯齿 |
| `0.3`（默认） | 中等拉直 | 每次最多跳过 30%，消除大部分锯齿但保留更多中间点 |
| `0.5`（最大值） | 较强拉直 | 每次最多跳过 50%，明显减少路径点 |

**示例**：

```
原始栅格路径（8 方向阶梯，15 个路径点）：
(0,0) → (1,1) → (2,2) → ... → (14,14)

straighten_factor=0.0：15 个路径点（与栅格路径相同）
straighten_factor=0.3：约 5 个路径点（中等拉直，默认值）
straighten_factor=0.5：约 3 个路径点（最大拉直）
```

**重要说明**：
- 拉直不会改变已计算的栅格路径代价（`total_cost`），代价仍然反映原始栅格路径
- 使用 Supercover 算法确保拉直后的路径 **绝不穿越 NODATA 或障碍物单元**
- 在障碍物边缘，拉直会保留必要的转弯路径点，确保路径绕过障碍
- `straighten_factor` 参数可通过工具箱界面调整

### 3.10 路径平滑——Chaikin 角切割

`smooth_path(path, iterations=3)` 对 **拉直后的路径** 进行后处理平滑。
...
- 默认 3 次迭代
- 如果路径只有 2 个点（直线），不做平滑
- **NODATA 安全保证**：平滑完成后，使用 Supercover 直线算法验证所有平滑线段不会穿越 NODATA / 障碍物单元。如果任何线段穿越了不可通行区域，将自动回退到拉直路径（不做平滑），确保输出路径永远不会穿过 NODATA 区域。

**实际意义**：经过拉直后，路径在必要的转弯处仍可能有锐角（例如绕过障碍物时）。Chaikin 平滑将这些锐角替换为平滑曲线，使输出更适合实际工程应用（如道路设计）。在 NODATA 区域密集的场景中，如果平滑会导致路径穿越不可通行区域，系统会自动跳过平滑步骤以保证路径安全性。
...
| 6 | Straighten Factor | GPDouble (0.00–0.50) | 数值输入（可选，默认 0.30） | 路径拉直因子 |
| 7 | Output Path | DEFeatureClass | 文件路径 | 输出路径要素类 |
...
### 6.6 `straighten_factor`（拉直因子）

- **范围**：0.00 – 0.50
- **默认值**：0.30（中等拉直）
- **精度**：支持小数点后两位的精细调节（例如 0.05、0.12、0.25 等）
- **含义**：控制路径拉直的激进程度。该参数决定拉直算法每次可以向前"看"多远来寻找可以直线连接的路径点。
  - `0.0`：**不拉直**。输出路径保留原始栅格路径的所有点，只沿 8 个方向移动。适用于需要保留原始栅格路径形态的场景。
  - `0.05–0.10`：**极轻微拉直**。每次最多跳过路径总长度的 5%–10%，仅消除最小范围的锯齿。适用于 NODATA 区域密集、希望路径紧贴栅格的场景。
  - `0.10–0.30`：**轻微至中等拉直**。消除大部分锯齿弯折，保留较多中间路径点。适用于一般场景。
  - `0.30–0.50`：**中等至较强拉直**。路径可以较自由地沿任意角度前进，显著减少路径点。
- **安全性**：无论 `straighten_factor` 取何值，拉直后的路径 **绝不穿越 NODATA 或障碍物单元**（使用 Supercover 直线算法保证）。
- **内部计算**：每次前瞻的最大步数 = `max(2, straighten_factor × (路径总点数 - 1))`
...
| 键名 | 类型 | 说明 |
|------|------|------|
| `path` | `list[(int, int)]` | 从起点到终点的栅格单元序列（8 方向连通），每个元素为 `(行, 列)` |
| `straightened_path` | `list[(float, float)]` | 经过视线拉直后的路径，仅保留必要的转弯路径点，允许任意角度前进 |
| `smoothed_path` | `list[(float, float)]` | 对拉直路径进行 Chaikin 平滑后的最终输出路径（保证不穿越 NODATA） |
| `total_cost` | `float` | 沿原始栅格路径的累计总代价 |
| `path_length` | `float` | 原始栅格路径的物理长度（地图单位） |
| `directions` | `list[int]` | 原始栅格路径每一步的方向编号（0–7），长度 = 路径点数 - 1 |
| `turning_angles` | `list[float]` | 原始栅格路径每个内部顶点处的转弯角度（度），长度 = 路径点数 - 2 |
...
用户输入
  ...
  ├─ 拉直因子 (straighten_factor)  [可选, 默认 1.0]
  ...
  └──────────────┬──────────────────────┘
          ...
          输出结果字典
            ├─ path (原始栅格路径，8 方向)
            ├─ straightened_path (拉直路径，任意角度)
            ├─ smoothed_path (拉直+平滑后的最终路径)
            ├─ total_cost (总代价)
            ├─ path_length (物理长度)
            ├─ directions (方向序列)
...
| NODATA 区域密集，需要保守拉直 | `straighten_factor=0.05–0.10`（极轻微拉直） |
| 希望路径尽量直但安全 | `straighten_factor=0.30–0.50`（中等至较强拉直） |
| 默认行为（中等拉直） | `straighten_factor=0.30` |
| 保留原始栅格路径形态 | `straighten_factor=0.0` |
